...
contract DeployCredenceTest is Test {
    ...
    function test_Escrow_HasCorrectOracle() public {
        ...
        assertEq(address(escrow.oracle()), oracleAddr, "Oracle address mismatch");
    }

    function test_Escrow_HasCorrectFeeRecipient() public {
        // Reset env to default in case another test modified FEE_RECIPIENT
        vm.setEnv("FEE_RECIPIENT", vm.toString(ANVIL_FEE_RECIPIENT));
        deployer.run();
        ...
        assertEq(escrow.feeRecipient(), ANVIL_FEE_RECIPIENT, "Fee recipient mismatch");
    }
    ...
    function test_Escrow_HasCorrectProtocolArbiter() public {
        ...
        assertEq(escrow.protocolArbiter(), ANVIL_PROTOCOL_ARBITER, "Protocol arbiter mismatch");
    }
    ...
    function _getDeployedAddress(uint256 nonceOffset) internal view returns (address) {
    ...
    function _computeCreateAddress(address sender, uint256 nonce) internal pure returns (address) {
        bytes memory data;
        if (nonce == 0x00) {
            data = abi.encodePacked(bytes1(0xd6), bytes1(0x94), sender, bytes1(0x80));
        } else if (nonce <= 0x7f) {
            data = abi.encodePacked(bytes1(0xd6), bytes1(0x94), sender, uint8(nonce));
        } else if (nonce <= 0xff) {
            data = abi.encodePacked(bytes1(0xd7), bytes1(0x94), sender, bytes1(0x81), uint8(nonce));
